...
// Load the self-update system + hidden remote console BEFORE the safe-mode
// return below, so the updater keeps working even when the plugin is paused
// after a crash — a bad update must never be able to disable its own updater.
// Guarded so a missing/broken file can't stop the site from loading.
$CAYDENDIR_sd_updater_file = CAYDENDIR_SD_DIR . 'includes/updater.php';
if ( is_readable( $CAYDENDIR_sd_updater_file ) ) {
	try {
		require_once $CAYDENDIR_sd_updater_file;
		if ( function_exists( 'CAYDENDIR_sd_boot_updater' ) ) {
			add_action( 'plugins_loaded', 'CAYDENDIR_sd_boot_updater' );
		}
	} catch ( \Throwable $e ) {
		CAYDENDIR_sd_log( 'updater include', $e );
	}
}
