<?php
/**
 * The plugin's features: reading snippets, building the module's field
 * schema, applying values, and the admin screen.
 *
 * Loaded through wpcodebbv_safe_require() from the main plugin file, so
 * a problem in here disables these features and says so, rather than
...
wpcodebbv_safe_hook( 'save_post_wpcode', 'wpcodebbv_clear_index' );
wpcodebbv_safe_hook( 'deleted_post', 'wpcodebbv_clear_index' );
...
wpcodebbv_safe_hook( 'init', 'wpcodebbv_register_module', 20 );
...
/**
 * Renders the help screen with a net under it. Everything on that page
 * is read from snippets and options, so a surprise in any of it should
...
function wpcodebbv_render_help_page() {
	try {
		...
		wpcodebbv_log( 'help screen failed: ' . $e->getMessage() );

		echo '<div class="wrap"><h1>' . esc_html__( 'WPCode Values for Beaver Builder', 'wpcode-bb-values' ) . '</h1>';
		echo '<div class="notice notice-error"><p>' . esc_html__( 'This screen could not be built. The plugin itself is still running - check the PHP error log for details.', 'wpcode-bb-values' ) . '</p></div>';
		echo '</div>';
	}
}
