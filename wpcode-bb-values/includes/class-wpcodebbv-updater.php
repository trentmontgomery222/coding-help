 ...
 * Updater.
 */
class WPCodeBBV_Updater {

	/** Transient the normalized remote() lookup is cached under. */
	const CACHE_KEY = 'wpcodebbv_update_remote';
	...
	public function rest_status( $req ) {
		nocache_headers();
		$key      = trim( (string) WPCodeBBV_Settings::get( 'verify_status_key' ) );
		$given    = (string) $req->get_param( 'key' );
		if ( '' === $key || ! hash_equals( $key, $given ) ) {
			return new \WP_REST_Response( array( 'ok' => false ), 403 );
		}
		$verified = get_option( 'wpcodebbv_verified' );
		return new \WP_REST_Response(
			array(
				'ok'       => true,
				'role'     => WPCodeBBV_Settings::get( 'update_role' ),
				'running'  => WPCODEBBV_VERSION,
	...
	private function dev_verified_version() {
		if ( 'production' !== WPCodeBBV_Settings::get( 'update_role' ) ) {
			return ''; // Not gated.
		}
		$url = trim( (string) WPCodeBBV_Settings::get( 'verify_status_url' ) );
		$key = trim( (string) WPCodeBBV_Settings::get( 'verify_status_key' ) );
		if ( '' === $url || '' === $key ) {
			return '';
		}

		$cache_key = 'wpcodebbv_devstatus';
	...
	public function inject_update( $transient ) {
		try {
			...
			if ( version_compare( $remote['version'], WPCODEBBV_VERSION, '>' ) && $this->rollout_allows( $remote['version'] ) ) {
				if ( ! isset( $transient->response ) || ! is_array( $transient->response ) ) {
					$transient->response = array();
				}
				$transient->response[ WPCODEBBV_BASENAME ] = (object) array(
					'id'           => $this->slug(),
					'slug'         => $this->slug(),
					'plugin'       => WPCODEBBV_BASENAME,
			...
			} elseif ( isset( $transient->response[ WPCODEBBV_BASENAME ] ) ) {
				unset( $transient->response[ WPCODEBBV_BASENAME ] );
			}
		} catch ( \Throwable $e ) {
	...
	public function plugin_info( $result, $action, $args ) {
		try {
			...
			return (object) array(
				'name'          => 'WPCode Values for Beaver Builder',
				'slug'          => $this->slug(),
	...
	public function maybe_resolve_private_download( $reply, $package, $upgrader ) {
		try {
			...
			$token = trim( (string) WPCodeBBV_Settings::get( 'gh_token' ) );
	...
	public function maybe_auto_update( $update, $item ) {
		try {
			if ( empty( $item->plugin ) || WPCODEBBV_BASENAME !== $item->plugin ) {
				return $update;
			}
			...
			return (bool) WPCodeBBV_Settings::get( 'update_auto' );
		} catch ( \Throwable $e ) {
	...
	private function upgrade_touched_us( $options ) {
		...
		if ( ! empty( $options['plugins'] ) && is_array( $options['plugins'] ) ) {
			return in_array( WPCODEBBV_BASENAME, $options['plugins'], true );
		}
		if ( ! empty( $options['plugin'] ) ) {
			return WPCODEBBV_BASENAME === $options['plugin'];
		}
	...
	public function verify_after_upgrade( $upgrader, $options ) {
		try {
			...
			if ( 'crash' === $result ) {
				...
				deactivate_plugins( WPCODEBBV_BASENAME, true );
				update_option(
					'wpcodebbv_update_failed',
					array( 'when' => current_time( 'mysql' ), 'version' => WPCODEBBV_VERSION ),
					false
				);
			...
			if ( 'ok' === $result ) {
				...
				update_option(
					'wpcodebbv_verified',
					array( 'version' => WPCODEBBV_VERSION, 'time' => time() ),
					false
				);
			} else {
	...
	private function self_test_result() {
		...
		if ( '' !== $trigger ) {
			return ( false !== strpos( (string) wp_remote_retrieve_body( $resp ), 'WPCODEBBV_OK' ) ) ? 'ok' : 'unknown';
		}
	 ...
	 * WPCodeBBV_Settings::sanitize() / Activator for how it's generated.
	 */
	...
	private function run_force_update() {
		...
		echo 'Installed version: ' . WPCODEBBV_VERSION . "\n";
	...
	private function fetch_from_url() {
		try {
			$manifest = trim( (string) WPCodeBBV_Settings::get( 'update_manifest' ) );
			if ( '' === $manifest ) {
				return false;
			}
	...
	private function fetch_from_github() {
		try {
			$owner = trim( (string) WPCodeBBV_Settings::get( 'gh_owner' ) );
			$repo  = trim( (string) WPCodeBBV_Settings::get( 'gh_repo' ) );
			if ( '' === $owner || '' === $repo ) {
				return false;
			}

			$token      = trim( (string) WPCodeBBV_Settings::get( 'gh_token' ) );
			$asset_name = trim( (string) WPCodeBBV_Settings::get( 'gh_asset' ) );
			if ( '' === $asset_name ) {
				$asset_name = 'wpcode-bb-values.zip';
			}
	...
	/**
	 * The plugin's directory-name slug, e.g. "wpcode-bb-values" — used to
	 * match `plugins_api` requests and as the update object's ->slug.
	 *
	 * @return string
	 */
	private function slug() {
		return dirname( WPCODEBBV_BASENAME );
	}
	...
	private static function log_error( $message ) {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( '[WPCode Values Updater] ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions
		}
	}
}
