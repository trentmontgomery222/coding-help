> **As ported into this plugin.** The tokens from §5 Step 0 were resolved as
> follows, and the class was de-namespaced to match this plugin's plain-class
> style (`WPCodeBBV_Scanner`, `WPCodeBBV_Module`):
>
> | Thing | Site Toolkit | Here |
> |---|---|---|
> | Namespace | `ACPS\SiteToolkit` | *(none)* — `WPCodeBBV_` class prefix |
> | Updater class | `Updater` | `WPCodeBBV_Updater` |
> | Settings class | `Settings` | `WPCodeBBV_Settings` |
> | Version constant | `ACPS_ST_VERSION` | `WPCODEBBV_VERSION` |
> | Basename constant | `ACPS_ST_BASENAME` | `WPCODEBBV_BASENAME` |
> | Path constant | `ACPS_ST_PATH` | `WPCODEBBV_DIR` |
> | REST namespace | `acps-st/v1` | `wpcode-bb-values/v1` |
> | Settings option | `acps_st_settings` | `wpcodebbv_settings` |
> | Option/transient prefix | `acps_st_` | `wpcodebbv_` |
> | Crash-test marker | `ACPS_OK` | `WPCODEBBV_OK` |
> | Force-update query var | `acps_st_update` | `wpcodebbv_update` |
> | Hidden-UI query var | `acps_updates` | `wpcodebbv_updates` |
>
> The Updates screen is at **Tools > WPCode Values** with `?wpcodebbv_updates=1`
> on the URL. Steps 1-8 are all done; the file below is the general guide,
> unchanged, for porting this to the next plugin.
