 ...
 * setting:
 *
 *   'url'    A small JSON manifest at a URL the site owner controls.
 *   'github' The GitHub Releases API for a given owner/repo.
 *
 * Both normalize to the same internal shape in remote() so every other method —
 * the update-transient injection, the "View details" popup, the private-asset
 * download fix, and the secret force-update URL — is source-agnostic.
 *
 * Every public hook callback is wrapped in try/catch: a broken update source
 * must never be able to break the rest of the site. Failures are logged only
 * when WP_DEBUG is on, and are cached briefly so a broken source isn't hammered
 * on every page load.
 *
 ...
 * ACPS_MC_Updater.
 */
class ACPS_MC_Updater {

	/** Transient the normalized remote() lookup is cached under. */
	const CACHE_KEY = 'acps_mc_update_remote';
	...
	/** REST namespace the /update-status route is registered under. */
	const REST_NS = 'acps-mc/v1';

	/** Query var the secret force-update URL is matched on. */
	const QUERY_VAR = 'acps_mc_update';

	/** Marker printed by the crash-test self-test responder. */
	const MARKER = 'ACPS_MC_OK';
	...
	public function register() {
		if ( ! ACPS_MC_Settings::get( 'update_enabled' ) ) {
			return;
		}
	...
	public function rest_status( $req ) {
		nocache_headers();
		$key   = trim( (string) ACPS_MC_Settings::get( 'verify_status_key' ) );
		$given = (string) $req->get_param( 'key' );
		if ( '' === $key || ! hash_equals( $key, $given ) ) {
			return new WP_REST_Response( array( 'ok' => false ), 403 );
		}
		$verified = get_option( 'acps_mc_verified' );
		return new WP_REST_Response(
			array(
				'ok'       => true,
				'role'     => ACPS_MC_Settings::get( 'update_role' ),
				'running'  => ACPS_MC_VERSION,
	...
	private function dev_verified_version() {
		if ( 'production' !== ACPS_MC_Settings::get( 'update_role' ) ) {
			return ''; // Not gated.
		}
		$url = trim( (string) ACPS_MC_Settings::get( 'verify_status_url' ) );
		$key = trim( (string) ACPS_MC_Settings::get( 'verify_status_key' ) );
		if ( '' === $url || '' === $key ) {
			return '';
		}

		$cache_key = 'acps_mc_devstatus';
	...
	public function inject_update( $transient ) {
		try {
			...
			if ( version_compare( $remote['version'], ACPS_MC_VERSION, '>' ) && $this->rollout_allows( $remote['version'] ) ) {
				if ( ! isset( $transient->response ) || ! is_array( $transient->response ) ) {
					$transient->response = array();
				}
				$transient->response[ ACPS_MC_BASENAME ] = (object) array(
					'id'           => $this->slug(),
					'slug'         => $this->slug(),
					'plugin'       => ACPS_MC_BASENAME,
				...
				if ( isset( $transient->no_update[ ACPS_MC_BASENAME ] ) ) {
					unset( $transient->no_update[ ACPS_MC_BASENAME ] );
				}
			} elseif ( isset( $transient->response[ ACPS_MC_BASENAME ] ) ) {
				unset( $transient->response[ ACPS_MC_BASENAME ] );
			}
		} catch ( \Throwable $e ) {
	...
	/**
	 * Supply the "View details" popup data. Matched by ->slug, the same way
	 * wordpress.org-hosted plugins are.
	 *
	...
	public function plugin_info( $result, $action, $args ) {
		try {
			...
			return (object) array(
				'name'          => 'ACPS Unused Media Cleanup',
				'slug'          => $this->slug(),
				...
				'requires'      => ! empty( $remote['requires_wp'] ) ? $remote['requires_wp'] : '5.6',
				'requires_php'  => ! empty( $remote['requires_php'] ) ? $remote['requires_php'] : '7.2',
	...
	/**
	 * Private-GitHub-asset download fix. WordPress core normally downloads
	 * `package` itself; a private release asset instead needs an authenticated
	 * request to resolve GitHub's signed (unauthenticated) redirect, then a plain
	 * download of THAT url — GitHub rejects a forwarded Authorization header on
	 * the signed S3 link.
	 *
	...
	public function maybe_resolve_private_download( $reply, $package, $upgrader ) {
		try {
			...
			$token = trim( (string) ACPS_MC_Settings::get( 'gh_token' ) );
			...
			$resp = wp_remote_get(
				...
				array(
					...
					'headers'     => array(
						...
						'User-Agent'    => 'ACPS-Media-Cleanup-Updater',
					),
				)
			);
			if ( is_wp_error( $resp ) ) {
	 ...
	 * background auto-update cron, honoring the "auto" setting. Every other
	 * plugin's auto-update decision passes through untouched.
	 *
	...
	public function maybe_auto_update( $update, $item ) {
		try {
			if ( empty( $item->plugin ) || ACPS_MC_BASENAME !== $item->plugin ) {
				return $update;
			}
			...
			return (bool) ACPS_MC_Settings::get( 'update_auto' );
		} catch ( \Throwable $e ) {
	...
	private function upgrade_touched_us( $options ) {
		...
		if ( ! empty( $options['plugins'] ) && is_array( $options['plugins'] ) ) {
			return in_array( ACPS_MC_BASENAME, $options['plugins'], true );
		}
		if ( ! empty( $options['plugin'] ) ) {
			return ACPS_MC_BASENAME === $options['plugin'];
		}
	...
	public function verify_after_upgrade( $upgrader, $options ) {
		try {
			...
			if ( 'crash' === $result ) {
				...
				deactivate_plugins( ACPS_MC_BASENAME, true );
				update_option(
					'acps_mc_update_failed',
					array( 'when' => current_time( 'mysql' ), 'version' => ACPS_MC_VERSION ),
					false
				);
			...
			if ( 'ok' === $result ) {
				delete_option( 'acps_mc_update_failed' );
				...
				update_option(
					'acps_mc_verified',
					array( 'version' => ACPS_MC_VERSION, 'time' => time() ),
					false
				);
			} else {
	...
	private function self_test_result() {
		$trigger = trim( (string) ACPS_MC_Settings::get( 'update_trigger' ) );
		$url     = '' !== $trigger
		...
		if ( '' !== $trigger ) {
			return ( false !== strpos( (string) wp_remote_retrieve_body( $resp ), self::MARKER ) ) ? 'ok' : 'unknown';
		}
	...
	public function maybe_show_update_failed_notice() {
		...
		$failed = get_option( 'acps_mc_update_failed' );
		if ( ! is_array( $failed ) ) {
			return;
		}
		echo '<div class="notice notice-error is-dismissible"><p>'
			. esc_html__( 'ACPS Unused Media Cleanup: a recent update failed its load test and was rolled back / kept disabled to protect the site.', 'acps-media-cleanup' )
	 ...
	 * Settings::sanitize() / activation for how it's generated.
	 */
	 ...
	 * @return array|false Normalized release info, or false on failure / not configured.
	 */
	public function remote( $force = false ) {
		...
		$source = ACPS_MC_Settings::get( 'update_source', 'url' );
	...
	public static function peek_status() {
		...
		$has_update = $remote && ! empty( $remote['version'] ) && version_compare( $remote['version'], ACPS_MC_VERSION, '>' );
		return array(
			'checked'    => true,
			'remote'     => $remote,
	...
	/**
	 * The `url` source: a small hosted JSON manifest.
	 *
	 * @return array|false
	 */
	private function fetch_from_url() {
		try {
			$manifest = trim( (string) ACPS_MC_Settings::get( 'update_manifest' ) );
			if ( '' === $manifest ) {
				return false;
			}

			$key = trim( (string) ACPS_MC_Settings::get( 'update_manifest_key' ) );
			if ( '' !== $key ) {
	...
	/**
	 * The `github` source: GitHub Releases API.
	 *
	 * @return array|false
	 */
	private function fetch_from_github() {
		try {
			$owner = trim( (string) ACPS_MC_Settings::get( 'gh_owner' ) );
			$repo  = trim( (string) ACPS_MC_Settings::get( 'gh_repo' ) );
			if ( '' === $owner || '' === $repo ) {
				return false;
			}

			$token      = trim( (string) ACPS_MC_Settings::get( 'gh_token' ) );
			$asset_name = trim( (string) ACPS_MC_Settings::get( 'gh_asset' ) );
			if ( '' === $asset_name ) {
				$asset_name = 'acps-media-cleanup.zip';
			}
	...
	/**
	 * The plugin's directory-name slug, e.g. "acps-media-cleanup" — used to
	 * match `plugins_api` requests and as the update object's ->slug.
	 *
	 * @return string
	 */
	private function slug() {
		return dirname( ACPS_MC_BASENAME );
	}

	/**
	 * Build the secret force-update URL for display in Settings. Empty until a
	 * trigger secret exists.
	 *
	 * @return string
	 */
	...
	/**
	 * Log a failure without ever throwing. Routes through the plugin's shared
	 * logger (WP_DEBUG-gated) when available.
	 *
	 * @param string $message Message to log.
	 */
	private static function log_error( $message ) {
		if ( function_exists( 'acps_mc_log' ) ) {
			acps_mc_log( 'Updater: ' . $message );
		} elseif ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'error_log' ) ) {
			error_log( '[ACPS Media Cleanup Updater] ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
		}
	}
}
