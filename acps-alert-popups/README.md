...
stop working, and its settings, wording and archive will be removed. If the
reason is a problem, the plugin does not need to go: the alert can be switched
off from Site Alerts, and how it behaves changed in Settings. So the warning
offers **Open Site Alerts Settings** as the first choice; **Delete anyway** goes
ahead with WordPress's normal delete. Nothing is added to the Plugins screen
otherwise.
