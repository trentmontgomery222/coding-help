...
class ACPS_Alerts_Settings {
	...
	public static function sanitize( array $input ) {
		...
		// Each screen saves only its own half. The maintenance screen posts none
		// of the ordinary settings, so its save keeps them exactly as stored —
		// without this, saving it reset the Main CSS, the cut-off and the rest
		// to their defaults.
		if ( ! empty( $input['_maintenance'] ) ) {
			$stored = self::all();

			foreach ( self::visible_keys() as $key ) {
				$clean[ $key ] = $stored[ $key ];
			}

			return array_merge( $clean, self::sanitize_maintenance( $input ) );
		}
