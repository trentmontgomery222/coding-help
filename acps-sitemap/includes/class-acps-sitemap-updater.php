<?php
/**
 * Self-hosted plugin updater (see UPDATE-SYSTEM.md).
 *
 ...
 * Updater.
 */
class ACPS_Sitemap_Updater {

	/** Transient the normalized remote() lookup is cached under. */
	const CACHE_KEY = 'acps_sitemap_update_remote';
	...
	const QUERY_VAR = 'acps_sitemap_update';
	...
	public function register() {
		if ( ! ACPS_Sitemap::get_setting( 'update_enabled' ) ) {
			return;
		}
	...
	public function rest_status( $req ) {
		nocache_headers();
		$key      = trim( (string) ACPS_Sitemap::get_setting( 'verify_status_key' ) );
		$given    = (string) $req->get_param( 'key' );
		if ( '' === $key || ! hash_equals( $key, $given ) ) {
			return new \WP_REST_Response( array( 'ok' => false ), 403 );
		}
		$verified = get_option( 'acps_sitemap_verified' );
		return new \WP_REST_Response(
			array(
				'ok'       => true,
				'role'     => ACPS_Sitemap::get_setting( 'update_role' ),
				'running'  => ACPS_SITEMAP_VERSION,
	...
	private function dev_verified_version() {
		if ( 'production' !== ACPS_Sitemap::get_setting( 'update_role' ) ) {
			return ''; // Not gated.
		}
		$url = trim( (string) ACPS_Sitemap::get_setting( 'verify_status_url' ) );
		$key = trim( (string) ACPS_Sitemap::get_setting( 'verify_status_key' ) );
		if ( '' === $url || '' === $key ) {
			return '';
		}

		$cache_key = 'acps_sitemap_devstatus';
	...
	public function fix_source_dir( $source, $remote_source, $upgrader, $args = array() ) {
		try {
			...
			if ( ACPS_SITEMAP_BASENAME !== $plugin ) {
				return $source;
			}
	...
	public function inject_update( $transient ) {
		try {
			...
			if ( version_compare( $remote['version'], ACPS_SITEMAP_VERSION, '>' ) && $this->rollout_allows( $remote['version'] ) ) {
				if ( ! isset( $transient->response ) || ! is_array( $transient->response ) ) {
					$transient->response = array();
				}
				$transient->response[ ACPS_SITEMAP_BASENAME ] = (object) array(
					'id'           => $this->slug(),
					'slug'         => $this->slug(),
				...
				if ( isset( $transient->no_update[ ACPS_SITEMAP_BASENAME ] ) ) {
					unset( $transient->no_update[ ACPS_SITEMAP_BASENAME ] );
				}
			} elseif ( isset( $transient->response[ ACPS_SITEMAP_BASENAME ] ) ) {
				unset( $transient->response[ ACPS_SITEMAP_BASENAME ] );
			}
		} catch ( \Throwable $e ) {
	...
	public function maybe_resolve_private_download( $reply, $package, $upgrader ) {
		try {
			...
			$token = trim( (string) ACPS_Sitemap::get_setting( 'gh_token' ) );
			...
			$resp = wp_remote_get(
				...
				array(
					...
					'headers'     => array(
						...
						'User-Agent'    => 'ACPS-Sitemap-Updater',
					),
				)
			);
			if ( is_wp_error( $resp ) ) {
	...
	public function maybe_auto_update( $update, $item ) {
		try {
			if ( empty( $item->plugin ) || ACPS_SITEMAP_BASENAME !== $item->plugin ) {
				return $update;
			}
			...
			return (bool) ACPS_Sitemap::get_setting( 'update_auto' );
		} catch ( \Throwable $e ) {
	...
	private function upgrade_touched_us( $options ) {
		...
		if ( ! empty( $options['plugins'] ) && is_array( $options['plugins'] ) ) {
			return in_array( ACPS_SITEMAP_BASENAME, $options['plugins'], true );
		}
		if ( ! empty( $options['plugin'] ) ) {
			return ACPS_SITEMAP_BASENAME === $options['plugin'];
		}
	...
	public function verify_after_upgrade( $upgrader, $options ) {
		try {
			...
			if ( ! is_plugin_active( ACPS_SITEMAP_BASENAME ) ) {
				activate_plugin( ACPS_SITEMAP_BASENAME, '', false, true );
			}
			...
			if ( 'crash' === $result ) {
				...
				update_option(
					...
					array( 'when' => current_time( 'mysql' ), 'version' => ACPS_SITEMAP_VERSION ),
					false
				);
			...
			if ( 'ok' === $result ) {
				...
				update_option(
					'acps_sitemap_verified',
					array( 'version' => ACPS_SITEMAP_VERSION, 'time' => time() ),
					false
				);
			} else {
	...
	private function self_test_result() {
		...
		if ( '' !== $trigger ) {
			return ( false !== strpos( (string) wp_remote_retrieve_body( $resp ), 'ACPS_SITEMAP_OK' ) ) ? 'ok' : 'unknown';
		}
	...
	public function maybe_show_update_failed_notice() {
		...
		echo '<div class="notice notice-error is-dismissible"><p>'
			. esc_html__( 'ACPS Sitemap: a recent update failed its load test and was rolled back / kept disabled to protect the site.', 'acps-sitemap' )
	...
	public function remote( $force = false ) {
		...
		$source = ACPS_Sitemap::get_setting( 'update_source', 'url' );
	...
	public static function peek_status() {
		...
		$has_update = $remote && ! empty( $remote['version'] ) && version_compare( $remote['version'], ACPS_SITEMAP_VERSION, '>' );
		return array(
			'checked'    => true,
			'remote'     => $remote,
	...
	private function fetch_from_url() {
		try {
			$manifest = trim( (string) ACPS_Sitemap::get_setting( 'update_manifest' ) );
			if ( '' === $manifest ) {
				return false;
			}
	...
	private function fetch_from_github() {
		try {
			$owner = trim( (string) ACPS_Sitemap::get_setting( 'gh_owner' ) );
			$repo  = trim( (string) ACPS_Sitemap::get_setting( 'gh_repo' ) );
			if ( '' === $owner || '' === $repo ) {
				return false;
			}

			$token      = trim( (string) ACPS_Sitemap::get_setting( 'gh_token' ) );
			$asset_name = trim( (string) ACPS_Sitemap::get_setting( 'gh_asset' ) );
			if ( '' === $asset_name ) {
				$asset_name = 'acps-sitemap.zip';
			}
	...
	/**
	 * The plugin's directory-name slug, e.g. "acps-sitemap" — used to
	 * match `plugins_api` requests and as the update object's ->slug.
	 *
	 * @return string
	 */
	private function slug() {
		return dirname( ACPS_SITEMAP_BASENAME );
	}
	...
	private static function log_error( $message ) {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( '[ACPS Sitemap Updater] ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions
		}
	}
}
